...
- **`Strategy`** (`engine/strategy.js`) — Parametrized by `(asset, windowMins)`. Core signal generator using `impliedProbability()` (N(d2) binary option model). **Dynamic strike price** fetched from Chainlink at window open; falls back to first Binance tick. Single signal mode: latency-arb — EMA-smoothed edge > dynamic threshold AND contract stale >1s behind spot.
  ...
  - **Calibration**: `calibration?.adjust(rawModelProb)` applied before edge calculation when `CALIBRATION_ENABLED=true` and sufficient history exists. Falls back to raw N(d2) if disabled or data is insufficient.
...
- **`Executor`** (`execution/executor.js`) — Places orders, monitors positions via event-driven book updates. Entry flow: `_selectOrderStrategy()` chooses maker (wide spread + time) or taker → place order → poll for fill → open position. Maker orders reprice up to 2× before falling through to taker. Partial entry fills cancel the remainder and open for the filled portion only. Exit flow: place sell → confirm fill → book realized P&L. Partial exit fills call `risk.applyPartialClose()` and retry next cycle. On rotation, `cancelOrdersForLabel(label)` cancels only that market's orders. On shutdown (`cancelAllOrders`), all positions are closed at mark-to-market with `estimated: true` flag for audit-trail clarity. Safety timeout (`MAX_HOLD_MS + SAFETY_BUFFER_MS`) force-closes at mark if exit remains unconfirmed, with an alert for manual verification.
  - **Event-driven monitoring**: primary exit path is `onBookUpdate(tokenId, book)` — called by `ArbEngine` on every Polymarket book event for tokens with open positions. Checks exit conditions immediately on each tick. Safety-poll (`SAFETY_POLL_MS = 500ms`) REST fallback fires when WS book events are stale (> 400ms gap). `MONITOR_INTERVAL_MS` is now used only as the maker reprice timeout, not as a monitoring poll interval.
...
- **`CalibrationTable`** (`engine/calibration.js`) — Binned correction for raw BS N(d2) probabilities. 10 bins across [0, 1]; each bin tracks observed win rate from historical trades. `adjust(modelProb)` blends raw and calibrated with weight `min(observations/50, 0.5)` — conservative, ramps to max 50%. Built from `data/features.ndjson` + `data/trades.ndjson` via `CalibrationTable.fromHistory()`. Requires 200+ fired signals to activate. **Opt-in via `CALIBRATION_ENABLED=true`** (default `false`) — the single global table does not yet account for per-asset vol regimes, momentum, or liquidity; enable only after verifying it improves reliability on your collected data.
...
Key parameters: `ASSETS` (comma-separated, default `BTC`), `WINDOWS` (comma-separated minutes, default `5`), `BANKROLL` (default 1300), `ENTRY_THRESHOLD` (default 8%, applied to 5m markets), `ENTRY_THRESHOLD_15M` (default 4%, applied to 15m+ markets — lower because 15m contracts stay near 50¢ longer with better depth), `MAX_BET_FRACTION` (max 10%), `MAX_OPEN_POSITIONS` (default 8), `DAILY_LOSS_LIMIT` (default $200 — blocks when bankroll falls more than this below today's intraday high), `PROFIT_TARGET_PCT` (default 8%), `STOP_LOSS_PCT` (default 15%), `ORDER_TYPE` (default GTC), `DRY_RUN`.
...
| `MONITOR_INTERVAL_MS` | 2000 | executor.js (maker reprice timeout) |
| `SAFETY_POLL_MS` | 500 | executor.js (REST fallback when WS stale) |
